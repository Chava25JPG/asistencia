...
def asignarRCMA():
    ...
    alumno_id = input("Ingrese el ID del maestro: ")

    # Verificar si el alumno existe en la base de datos
    cursor = conn.cursor()
    query = "SELECT * FROM maestro WHERE id = %s"
    ...
    if result is not None:
        ...
        print("Se ha asignado/reasignado la tarjeta RFID al Maestro correctamente.")
    else:
        # Si el alumno no existe, informar al usuario
        print("El ID de maestro ingresado no existe en la base de datos.")
